/**
 * RRS-2-BookingRequests-v1.4.26
 * BOOKING REQUESTS (new) -> Hold best slot (ranked choices, else smart fallback) + generate confirmation code + update request
 *
 * BUG FIXES: Populate BOOKINGS {Reservation Name} on provisional booking create (used downstream via lookups on SLOTS).
 *
 * BEHAVIOR:
 * Priority:
...
const paidQuery = await paid.selectRecordsAsync({
  fields: ["Attendee Age", "Attendee Code", "DCG Email", "First Name", "Last Name", "Studio Name"],
});
...
// Build Reservation Name (stable display label)
const firstName = (paidRec.getCellValueAsString("First Name") || "").trim();
const lastName = (paidRec.getCellValueAsString("Last Name") || "").trim();
const studioName = (paidRec.getCellValueAsString("Studio Name") || "").trim();
const reservationName = [firstName, lastName].filter(Boolean).join(" ").trim()
  ? `${[firstName, lastName].filter(Boolean).join(" ")}${studioName ? " — " + studioName : ""}`
  : (studioName || "Reservation");
...
await bookings.createRecordsAsync([{
  fields: {
