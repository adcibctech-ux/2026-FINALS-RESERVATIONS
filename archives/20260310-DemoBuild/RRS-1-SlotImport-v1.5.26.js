/**
 * RRS-1-SlotImport-v1.5.26: Bug fix — remove output.* (not supported in your Automation run environment); use console.log instead.
 *
 * Reconcile "MASTER SLOTS (Synced)" ➜ "SLOTS (Operational)"
 *
 * Creates missing Operational slots, updates links/times/room, and voids missing slots.
 * - BOOKED missing from master => NEEDS REVIEW
 * - everything else missing from master => VOIDED
 *
 * Assumes exact names:
 * Tables: "MASTER SLOTS (Synced)", "SLOTS (Operational)", "ROOMS"
 * Fields:
...
for (const [slotKey, m] of masterBySlotKey.entries()) {
  ...
  if (!roomId) missingRoomMatches += 1;
...
console.log(`- Missing room matches (Location not found in ROOMS): ${missingRoomMatches}`);
