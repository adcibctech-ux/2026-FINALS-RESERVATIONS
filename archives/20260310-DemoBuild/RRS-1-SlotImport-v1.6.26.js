/**
 * RRS-1-SlotImport-v1.6.26 — Bug fix: handle dateTime fields + skip creates that would produce blank primary (Operational Slot)
 *
 * Root cause fixed:
 * - {Start Time}/{End Time} are type dateTime, not date, so prior script skipped them.
 * - Primary field {Operational Slot} is a formula depending on Room + Start/End; blanks cause create failure.
 *
 * New behavior:
 * - Accepts both "dateTime" and "date" when writing Start/End.
 * - Requires a matching ROOMS record for the master {Location}; if not found, skips create + logs it.
 */
...
// ---------- Preflight (updated for dateTime) ----------
const DATE_TYPES = ["dateTime", "date"]; // accept both
...
const schemaChecks = [
  ...
  expectType(opTable, "Start Time", DATE_TYPES),
  expectType(opTable, "End Time", DATE_TYPES),
  expectType(opTable, "Status", ["singleSelect"]),

  // Master synced table
  expectType(masterTable, "Slot Key", ["formula", "singleLineText", "multilineText"]),
  expectType(masterTable, "Start Time", DATE_TYPES),
  expectType(masterTable, "End Time", DATE_TYPES),
  expectType(masterTable, "Location", ["singleSelect", "singleLineText", "multilineText"]),
];

const bad = schemaChecks.filter((c) => !c.ok);
if (bad.length) {
  console.log("⚠️ Slot Sync Preflight: schema mismatch detected");
  for (const b of bad) {
    console.log(`- Field "${b.name}" | actual type: ${b.type} | expected: ${b.expected}`);
  }
  console.log("Fix the field types above. Script will continue but will SKIP writing to mis-typed fields.");
} else {
  console.log("✅ Preflight OK: field types match expected schema.");
}
...
for (const [slotKey, m] of masterBySlotKey.entries()) {
  ...
  // STRICT: if no room match, skip create so primary formula doesn't resolve blank and crash create
  if (!roomId) {
    skippedNoRoomMatch += 1;
  ...
  safeSet(fields, opTable, "Room", [{ id: roomId }], ["multipleRecordLinks"]);
  safeSet(fields, opTable, "Start Time", m.start ?? null, DATE_TYPES);
  safeSet(fields, opTable, "End Time", m.end ?? null, DATE_TYPES);
...
for (const [slotKey, opRec] of opBySlotKey.entries()) {
  ...
  // Ensure room matches (only if room exists)
  const roomId = roomNameToId.get(m.location);
  if (roomId) {
    ...
    // If master location doesn't match a room, mark as NEEDS REVIEW (doesn't delete)
    // Only do this if field is writable.
    safeSet(fieldsToSet, opTable, "Status", "NEEDS REVIEW", ["singleSelect"]);
    console.log(`WARN: No ROOMS match for existing slot: Location="${m.location}" | Slot Key="${slotKey}"`);
  }

  // Copy times
  safeSet(fieldsToSet, opTable, "Start Time", m.start ?? null, DATE_TYPES);
  safeSet(fieldsToSet, opTable, "End Time", m.end ?? null, DATE_TYPES);
