/**
 * RRS-2-BookingRequests-v1.1.26
 * BOOKING REQUESTS (new) -> Hold best slot (ranked choices, else smart fallback) + generate confirmation code + update request
 *
 * BUG FIXES: Added validation — only process if PAID ROSTER {DCG Email} matches BOOKING REQUESTS {Reservation Email}; else fail with FAILED - INVALID EMAIL.
 *
 * BEHAVIOR:
 * Priority:
...
// 4–5 fallback: nonconflicting 60 then 30
if (!chosen) {
