/**
 * RRS-3-Confirmations-v1.10.26
 * BUG FIXES:
 * - Prevents "Record was specified twice" by merging updates per record id before updateRecordsAsync.
 *
 * NOTES:
 * - FAILED = stolen slot
 * - EXPIRED = timed out
 * - Auto-rematch once, creates new booking MATCHED and sets slot HELD + temp hold fields for 15 minutes
 */
...
// ---- merge updates by id to avoid duplicates ----
function mergeUpdates(updates) {
  ...
  for (const u of updates) {
    ...
    if (!prev) {
      m.set(u.id, { id: u.id, fields: { ...(u.fields || {}) } });
    } else {
      m.set(u.id, { id: u.id, fields: { ...(prev.fields || {}), ...(u.fields || {}) } });
    }
  }
...
for(const b of selectedCandidates){
  ...
  if(!alreadyRematched && paidLink?.id){
    ...
    if(newSlot){
      ...
      // mark original as rematch attempted for audit
      bookingUpdates.push({ id:b.id, fields:{ "Auto Rematch Attempted?": true }});
    }
  }
}

// Apply updates (deduped)
...
console.log(`RRS-3 v1.10.26 complete: result=${result}, confirmed=${confirmedBookings.length}, failed=${failedBookings.length}, cancelledUnselected=${toCancel.length}, newMatches=${newBookingIds.length}`);
