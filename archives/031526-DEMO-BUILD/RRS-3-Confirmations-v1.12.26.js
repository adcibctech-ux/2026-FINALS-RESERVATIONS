/**
 * RRS-3-Confirmations-v1.12.26
 *
 * NEW:
 * - When auto-rematch creates new MATCHED bookings:
 *    - Writes BOOKING REQUESTS.{Rematch Summary}
 *    - Checks BOOKING REQUESTS.{Rematch Email Needed}
 *    - (If BOOKING REQUESTS has {Match Expires At}) sets it to NOW()+15min for email display consistency
 *
 * RULES:
 * - FAILED = stolen slot (slot not available at confirm time) => CAN auto-rematch once
 * - EXPIRED = timed out (now > Match Expires At) => NO auto-rematch
 * - Auto-rematch creates NEW booking(s) MATCHED and sets slot Status=HELD + Temp Hold fields for 15 min
 * - Cancels unselected MATCHED bookings for same request/code/email
 *
 * SUMMARY FORMAT:
...
for(const b of selectedCandidates){
  ...
  if(!alreadyRematched && paidLink?.id){
    ...
    if(newSlot){
      ...
      // Mark original as rematch attempted (audit)
...
// Create rematch bookings + prepare New Match Summary + trigger BOOKING REQUEST email
let newBookingIds = [];
let newMatchLines = [];
...
if(rematchCreates.length){
  ...
  // Append new rematch bookings to BOOKING REQUESTS.{BOOKINGS}
  if(HAS_REQ_BOOKINGS){
  ...
  // Trigger consolidated rematch email from BOOKING REQUESTS
  const reqUpdate = {
    "Rematch Email Needed": true,
    "Rematch Summary": newMatchLines.join("\n"),
    "Rematch Email Sent At": null, // let the automation set it
  };

  // Also set Request Match Expires At for email display consistency (if you have it)
  if(HAS_REQ_MATCH_EXPIRES){
    const reqExpires = new Date(now.getTime() + REMATCH_MINUTES*60*1000);
    reqUpdate["Match Expires At"] = reqExpires;
  }
...
console.log(`RRS-3 v1.12.26 complete: result=${result}, confirmed=${confirmedBookings.length}, failed=${failedBookings.length}, cancelledUnselected=${toCancel.length}, newMatches=${newBookingIds.length}`);
