/**
 * RRS-3-Confirmations-v1.11.26
 * CONFIRMATIONS -> Confirm selected bookings + auto-rematch stolen confirmations (multi) with HELD slots
 *
 * BUG FIXES:
 * - Supports multi-rematch in one confirmation run (e.g., 4 stolen -> 4 new MATCHED bookings).
 * - Writes new rematch bookings into:
 *    CONFIRMATIONS.{New Match Booking(s)} + CONFIRMATIONS.{New Match Summary}
 *    BOOKING REQUESTS.{BOOKINGS} (appends)
 * - FAILED = stolen slot (slot not available at confirm time)
 * - EXPIRED = timed out (now > Match Expires At) (no rematch)
 * - Auto-rematch happens once per booking chain via {Auto Rematch Attempted?}
 * - Auto-rematch holds the new slot for 15 min:
 *    SLOTS.Status=HELD + Temp Hold Token/Expires/Email
 *
 * SUMMARY FORMAT:
...
// Required schema for this script
...
// ---- merge updates by id (prevents "specified twice") ----
...
// ------------------ Load request slot lists ------------------
const reqQuery = await requests.selectRecordsAsync({ fields:["Requested Slot(s)","Backup Slot(s)", ...(HAS_REQ_BOOKINGS?["BOOKINGS"]:[])] });
...
function isSlotEligibleForRematch(slotRec){
  // only rematch into AVAILABLE slots that aren't temp-held
...
for(const b of selectedCandidates){
  ...
  if(!alreadyRematched && paidLink?.id){
    ...
    if(newSlot){
      ...
      // Mark original as rematched attempted (audit)
...
// Create rematch bookings + write New Match fields
let newBookingIds = [];
let newMatchLines = [];
...
// Append new rematch bookings to BOOKING REQUESTS.{BOOKINGS} (so email views stay coherent)
if(HAS_REQ_BOOKINGS && newBookingIds.length){
  ...
  const mergedLinks = Array.from(existingIds).map(id=>({ id }));
  await requests.updateRecordAsync(reqRec.id, { "BOOKINGS": mergedLinks });
}
...
// Write confirmation record (includes new match outputs)
...
console.log(`RRS-3 v1.11.26 complete: result=${result}, confirmed=${confirmedBookings.length}, failed=${failedBookings.length}, cancelledUnselected=${toCancel.length}, newMatches=${newBookingIds.length}`);
