/**
 * RRS-8-CancellationProcess-v1.1.26
 * CANCELLATION REQUESTS (button) -> If APPROVED, cancel booking + release slots + revert MASTER SCHEDULE to AVAILABLE RESERVATION
 *
 * BUG FIXES:
 * - Populate {Processed By} from {Approved By}, and set {Processed At} to the script completion timestamp.
 *
 * BEHAVIOR:
 ...
 *    - Sets {Processed By} = {Approved By}
 *    - Sets {Processed At} = script finish timestamp
 */
...
// Capture Approved By to mirror into Processed By (single select value string)
const approvedBy = (cr.getCellValueAsString("Approved By") || "").trim();
...
// ---- Write System Notes + mark cancellation complete + processed metadata ----
const finishedAt = new Date(); // capture completion time
const existingNotes = (cr.getCellValueAsString("System Notes") || "").trim();

const auditLines = [
  `Processed at: ${finishedAt.toISOString()}`,
...
const cancelUpdate = {
  ...
  "System Notes": existingNotes
    ? `${existingNotes}\n\n---\n${auditLines.join("\n")}`
    : auditLines.join("\n"),
  "Processed At": finishedAt,
};

// Mirror Approved By -> Processed By, if present
if (approvedBy) {
  cancelUpdate["Processed By"] = sel(approvedBy);
}

await cancels.updateRecordAsync(cr.id, cancelUpdate);
