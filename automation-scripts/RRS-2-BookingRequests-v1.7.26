/**
 * RRS-2-BookingRequests-v1.7.26
 * BOOKING REQUESTS (new) -> Hold best slot (ranked choices, else smart fallback) + generate confirmation code + update request
 *
 * BUG FIXES: Populate BOOKING REQUESTS {Held Slot Summary} in plain English from held slot(s): Room // YYYY-MM-DD // Start (EST) // End (EST).
 *
 * BEHAVIOR:
 * Priority:
...
const confirmCode = randomCode(8);
...
const paidQuery = await paid.selectRecordsAsync({
  fields: ["Attendee Age", "Attendee Code", "DCG Email", "Reservation Name"],
});
...
const reservationName = (paidRec.getCellValueAsString("Reservation Name") || "").trim() || "Reservation";
...
const attendeeCode = (paidRec.getCellValueAsString("Attendee Code") || "").trim();
...
const slotUpdates = heldLinks.map(l => ({
  id: l.id,
  fields: {
    "Status": sel("HELD"),
...
// ------------------ Build Held Slot Summary from held slot(s) (NEW) ------------------
const summaryLines = [];
for (const l of heldLinks) {
...
const heldSlotSummary = summaryLines.join("\n");

// ------------------ Upsert provisional BOOKING (no duplicates) ------------------
const bookingQuery = await bookings.selectRecordsAsync({
  fields: ["Booking Request"],
});
...
if (existingBooking) {
  await bookings.updateRecordAsync(existingBooking.id, bookingFields);
} else {
  await bookings.createRecordsAsync([{ fields: bookingFields }]);
}
...
await requests.updateRecordAsync(req.id, {
  ...
  "Held Slot Summary": heldSlotSummary,
});
